    ...
    <div class="kt-container  kt-container--fluid ">
        <div class="kt-subheader__main">
            <h3 class="kt-subheader__title">
                   Task Name &nbsp; : &nbsp;<strong style="color: #FF9800;">{{ $seo_task->task_name }}</strong></h3>
        </div>
        <div class="kt-subheader__toolbar">
            @if(!empty($seo_task->campaign))
            <a href="{{ route('campaign_details', $seo_task->campaign) }}" class="btn btn-brand btn-elevate btn-icon-sm"><i class="fa fa-undo"></i> Campaign Page</a>
            @endif
            <a href="{{ route('campaign_index') }}" class="btn btn-warning btn-elevate btn-icon-sm"><i class="fa fa-undo"></i> Campagins</a>
            <a href="{{ route('seo_index') }}" class="btn btn-success btn-elevate btn-icon-sm"><i class="fa fa-undo"></i> Organic Task</a>
        </div>
    </div>
</div>
        ...
        <div class="kt-widget1 col-md-12" style="padding:20px 10px;">
            <table class="table table-bordered table-striped table-large">
                <tbody>
                    ...
                    <tr>
                        <td>Project</td>
                        <td>{{ $seo_task->project }}</td>
                    </tr>
                    <tr>
                        <td>Campaign</td>
                        <td>{{ $seo_task->campaign }}</td>
                    </tr>
                    <tr>
                        <td>Activity TYpe</td>
                        <td>{{ $seo_task->activity }}</td>
                    </tr>
                    <tr>
                        <td>Task Owner ETA</td>
                        <td>{{ $seo_task->task_owner_eta }}</td>
                    </tr>
                    <tr>
                        <td>Priority</td>
                        <td>{{ $seo_task->priority }}</td>
                    </tr>
                    <tr>
                        <td>Task Status</td>
                        <td>{{ $seo_task->status }}</td>
                    </tr>
                </tbody>
...
<form class="kt-form" method="post" action="{{ route('update_seo', $seo_task->id) }}">
    @csrf
    {{ method_field('PUT') }}
                <div class="form-group row">
                    <div class="col-md-12">
                        <label>Task Name</label>
                        <input type="text" title="Task Name" placeholder="" required="" maxlength="255" class="form-control" name="name" id="task_name" value="{{ $seo_task->name }}">
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <option {{ $seo_task->activity == "Backlink Building"? "selected": " " }} value="Backlink Building">Backlink Building</option>
                    <option {{ $seo_task->activity == "Image Submission"? "selected": " " }} value="Image Submission">Image Submission</option>
                    <option {{ $seo_task->activity == "Classsified Submission"? "selected": " " }} value="Classsified Submission">Classsified Submission</option>
                    <option {{ $seo_task->activity == "PPT/PDF Submission"? "selected": " " }} value="PPT/PDF Submission">PPT/PDF Submission</option>
                    <option {{ $seo_task->activity == "FB marketplace Posting"? "selected": " " }} value="FB marketplace Posting">FB marketplace Posting</option>
                    <option {{ $seo_task->activity == "FB Group Sharing"? "selected": " " }} value="FB Group Sharing">FB Group Sharing</option>
                    <option {{ $seo_task->activity == "Property Portals"? "selected": " " }} value="Property Portals">Property Portals</option>
                    <option {{ $seo_task->activity == "Infographics"? "selected": " " }} value="Infographics">Infographics</option>
                    <option {{ $seo_task->activity == "Minisites"? "selected": " " }} value="Minisites">Minisites</option>
                    <option {{ $seo_task->activity == "PBN Content Submission"? "selected": " " }} value="PBN Content Submission">PBN Content Submission</option>
                    <option {{ $seo_task->activity == "GBL ORM"? "selected": " " }} value="GBL ORM">GBL ORM</option>
                    <option {{ $seo_task->activity == "FB Ad Commenting"? "selected": " " }} value="FB Ad Commenting">FB Ad Commenting</option>
                    <option {{ $seo_task->activity == "Keyword Ranking"? "selected": " " }} value="Keyword Ranking">Keyword Ranking</option>
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select style="width: 100%" class="form-control kt-select2" id="select_campaign" required="" name="campaign">
                            ...
                            @if($seo_task->campaign == 0)
                            <option value="0" selected>Non Campaign Task</option>
                            @foreach($campaigns as $campaign)
                            <option {{ $seo_task->campaign == $campaign->id ? "selected": " " }}  value="{{ $campaign->id }}">{{ $campaign->name }}</option>
                            @endforeach
                            @else
                ...
                <div class="form-group row">
                    <div class="col-md-6">
                        ...
                        <div class="input-group date">
                            <input type="text" class="form-control" name="task_owner_eta" readonly placeholder="Select date & time" id="kt_datetimepicker_2" required="" value="{{ $seo_task->task_owner_eta }}" />
                    ...
                    <div class="col-md-6">
                        ...
                        <select style="width: 100%" class="form-control kt-select2" id="select_priority" required=""  name="priority">
                            <option value="">** </option>
                            <option {{ $seo_task->priority == "high" ? "selected": " " }} value="high">High</option>
                            <option {{ $seo_task->priority == "medium" ? "selected": " " }} value="medium">Medium</option>
                            <option {{ $seo_task->priority == "low" ? "selected": " " }} value="low">Low</option>
                        </select>
                    </div>
                </div>
                <div class="form-group row">
...
<!--begin::Portlet-->
<form class="kt-form" enctype="multipart/form-data" method="post" action="{{ route('update_seo_task', $seo_task->id) }}">
    @csrf
    {{ method_field('PUT') }}
...
<!--begin::Portlet-->
<form class="kt-form" enctype="multipart/form-data" method="post" action="{{ route('update_seo_sub_task', $seo_task->id) }}">
    @csrf
    {{ method_field('PUT') }}
<div class="kt-portlet">
    <div class="kt-portlet__head">
...
@section('footer_js')
    ...
    $(function() {
        ...
        @if($seo_task->status == 'internal_review')
        $('.creative-upload-block').show();
        @elseif($seo_task->status == 'external_review')
        $('.creative-upload-block').show();
        @elseif($seo_task->status == 'completed')
        $('.creative-upload-block').show();
        @else
