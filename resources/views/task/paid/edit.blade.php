...
<div class="kt-subheader   kt-grid__item" id="kt_subheader">
    <div class="kt-container  kt-container--fluid ">
        ...
        <div class="kt-subheader__toolbar">
            @if(!empty($paid_task->campaign))
            <a href="{{ route('campaign_details', $paid_task->campaign) }}" class="btn btn-brand btn-elevate btn-icon-sm"><i class="fa fa-undo"></i> Campaign Page</a>
            @endif
            <a href="{{ route('campaign_index') }}" class="btn btn-warning btn-elevate btn-icon-sm"><i class="fa fa-undo"></i> Campagins</a>
            <a href="{{ route('paid_index') }}" class="btn btn-success btn-elevate btn-icon-sm"><i class="fa fa-undo"></i> Paid Task</a>
        </div>
    </div>
</div>
        ...
        <div class="kt-widget1 col-md-12" style="padding:20px 10px;">
            <table class="table table-bordered table-striped table-success table-large">
                <tbody>
                    ...
                    <tr>
                        <td>Project</td>
                        <td>{{ $paid_task->project }}</td>
                    </tr>
                    <tr>
                        <td>Campaign</td>
                        <td>{{ $paid_task->campaign }}</td>
                    </tr>
                    <tr>
                        <td>Activity TYpe</td>
                        <td>{{ $paid_task->activity }}</td>
                    </tr>
                    <tr>
...
<form class="kt-form" method="post" action="{{ route('update_paid', $paid_task->id) }}">
    @csrf
    {{ method_field('PUT') }}
                ...
                <div class="form-group row">
                    ...
                    <option value="Optimization" {{ $paid_task->activity == "Optimization"? "selected": " " }}>Optimization</option>
                    <option value="Set-up" {{ $paid_task->activity == "Set-up"? "selected": " " }}>Set-up</option>
                    <option value="Reporting" {{ $paid_task->activity == "Reporting"? "selected": " " }}>Reporting</option>
                    <option value="Media Plan" {{ $paid_task->activity == "Media Plan"? "selected": " " }}>Media Plan</option>
                    <option value="Research" {{ $paid_task->activity == "Research"? "selected": " " }}>Research</option>                            
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select style="width: 100%" class="form-control kt-select2" id="select_campaign" required="" name="campaign">
                            ...
                            @if($paid_task->campaign == 0)
                            <option value="0" selected>Non Campaign Task</option>
                            @foreach($campaigns as $campaign)
                            <option {{ $paid_task->campaign == $campaign->id ? "selected": " " }}  value="{{ $campaign->id }}">{{ $campaign->name }}</option>
                            @endforeach
                            @else
...
<!--begin::Portlet-->
<form class="kt-form" enctype="multipart/form-data" method="post" action="{{ route('update_paid_sub_task', $paid_task->id) }}">
    @csrf
    {{ method_field('PUT') }}
<div class="kt-portlet">
    <div class="kt-portlet__head">
