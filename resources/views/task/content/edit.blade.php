    ...
    <div class="kt-container  kt-container--fluid ">
        <div class="kt-subheader__main">
            <h3 class="kt-subheader__title">
                   Task Name &nbsp; : &nbsp;<strong style="color: #FF9800;">{{ $content_task->task_name }}</strong></h3>
        </div>
        <div class="kt-subheader__toolbar">
            @if(!empty($content_task->campaign))
            <a href="{{ route('campaign_details', $content_task->campaign) }}" class="btn btn-brand btn-elevate btn-icon-sm"><i class="fa fa-undo"></i> Campaign Page</a>
            @endif
            <a href="{{ route('campaign_index') }}" class="btn btn-warning btn-elevate btn-icon-sm"><i class="fa fa-undo"></i> Campagins</a>
            <a href="{{ route('content_index') }}" class="btn btn-success btn-elevate btn-icon-sm"><i class="fa fa-undo"></i> Content Task</a>
        </div>
    </div>
</div>
        ...
        <div class="kt-widget1 col-md-12" style="padding:20px 10px;">
            <table class="table table-bordered table-striped table-large">
                <tbody>
                    <tr>
                        <td>Name</td>
                        <td>{{ $content_task->name }}</td>
                    </tr>
                    <tr>
                        <td>Project</td>
                        <td>{{ $content_task->project }}</td>
                    </tr>
                    <tr>
                        <td>Campaign</td>
                        <td>{{ $content_task->campaign }}</td>
                    </tr>
                    <tr>
                        <td>Activity TYpe</td>
                        <td>{{ $content_task->activity }}</td>
                    </tr>
                    <tr>
                        <td>Task Owner ETA</td>
                        <td>{{ $content_task->task_owner_eta }}</td>
                    </tr>
                    <tr>
                        <td>Priority</td>
                        <td>{{ $content_task->priority }}</td>
                    </tr>
                    <tr>
                        <td>Task Status</td>
                        <td>{{ $content_task->status }}</td>
                    </tr>
                </tbody>
            </table>
            <hr>
...
<form class="kt-form" method="post" action="{{ route('update_content', $content_task->id) }}">
    @csrf
    {{ method_field('PUT') }}
                <div class="form-group row">
                    <div class="col-md-12">
                        <label>Task Name</label>
                        <input type="text" title="Task Name" placeholder="" required="" maxlength="255" class="form-control" name="name" id="task_name" value="{{ $content_task->name }}">
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <option {{ $content_task->activity == "Blog Post Content"? "selected": " " }} value="Blog Post Content">Blog Post Content</option>
                    <option {{ $content_task->activity == "Creative Ad Content"? "selected": " " }} value="Creative Ad Content">Creative Ad Content</option>
                    <option {{ $content_task->activity == "Hero Message"? "selected": " " }} value="Hero Message">Hero Message</option>
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select style="width: 100%" class="form-control kt-select2" id="select_campaign" required="" name="campaign">
                            ...
                            @if($content_task->campaign == 0)
                            <option value="0" selected>Non Campaign Task</option>
                            @foreach($campaigns as $campaign)
                            <option {{ $content_task->campaign == $campaign->id ? "selected": " " }}  value="{{ $campaign->id }}">{{ $campaign->name }}</option>
                            @endforeach
                            @else
                ...
                <div class="form-group row">
                    <div class="col-md-6">
                        ...
                        <div class="input-group date">
                            <input type="text" class="form-control" name="task_owner_eta" readonly placeholder="Select date & time" id="kt_datetimepicker_2" required="" value="{{ $content_task->task_owner_eta }}" />
                    ...
                    <div class="col-md-6">
                        ...
                        <select style="width: 100%" class="form-control kt-select2" id="select_priority" required=""  name="priority">
                            <option value="">** </option>
                            <option {{ $content_task->priority == "high" ? "selected": " " }} value="high">High</option>
                            <option {{ $content_task->priority == "medium" ? "selected": " " }} value="medium">Medium</option>
                            <option {{ $content_task->priority == "low" ? "selected": " " }} value="low">Low</option>
                        </select>
                    </div>
                </div>
                <div class="form-group row">
...
@if(count($sub_task) != 0)
    ...
    <div class="col-md-6">
<!--begin::Portlet-->
<form class="kt-form" enctype="multipart/form-data" method="post" action="{{ route('update_content_task', $content_task->id) }}">
    @csrf
    {{ method_field('PUT') }}
        ...
        <div class="form-group row">
            <div class="col-md-6">
                ...
                <select style="width: 100%" class="form-control kt-select2" id="select_task_status" required="" name="status" ng-model="TaskStatus">
                    <option value=""> --- </option>
                    <option {{ $content_task->status == "new" ? "selected": " " }} value="new">New</option>
                    <option {{ $content_task->status == "New Updates" ? "selected": " " }} value="New Updates">New Updates</option>
                    <option {{ $content_task->status == "Correction Updates" ? "selected": " " }} value="Correction Updates">Correction Updates</option>
                    <option {{ $content_task->status == "Under Review" ? "selected": " " }} value="Under Review">Under Review</option>
                    <option {{ $content_task->status == "Completed" ? "selected": " " }} value="Completed">Completed</option>
                </select>
            </div>
            <div class="col-md-6">
                <label>Total Duration</label>
                    <input type="text" class="form-control" name="duration" placeholder="Total Duration" value="{{ $content_task->duration }}" />
            </div>
        </div>
                <div class="form-group row">
                    <div class="col-md-12">
                ...
                <textarea class="form-control" name="content" id="content" rows="10">{{ $content_task->content }}</textarea>
...
<!--begin::Portlet-->
<form class="kt-form" enctype="multipart/form-data" method="post" action="{{ route('update_content_sub_task', $content_task->id) }}">
    @csrf
    {{ method_field('PUT') }}
<div class="kt-portlet">
    <div class="kt-portlet__head">
...
@section('footer_js')
    ...
    $(function() {
        ...
        @if($content_task->status == 'internal_review')
        $('.creative-upload-block').show();
        @elseif($content_task->status == 'external_review')
        $('.creative-upload-block').show();
        @elseif($content_task->status == 'completed')
        $('.creative-upload-block').show();
        @else
